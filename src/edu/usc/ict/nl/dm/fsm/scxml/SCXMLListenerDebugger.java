...
public class SCXMLListenerDebugger extends Thread implements SCXMLListener {
	...
	public SCXMLListenerDebugger(String server,int port) {
		super();
		this.server=server;
		this.port=port;
		reconnect();
    	...
    	while (true) {
    		try {
    			...
    			if (!events.isEmpty() && checkConnection()) {
    				...
    				System.out.println("Debug listener, sending event: "+event);
    				out.println(event);
    			}
	...
	private boolean reconnect() {
		try {
			//System.out.println("Debug listener: reconnecting.");
