...
public class EnglishWrittenNumbers2Digits {
	...
	public static List<Token> parseWrittenNumbers(NLUConfig nluConfig,List<Token> tokens) throws Exception {
		File grammar=new File(Sanitizer.file(nluConfig.getNLUContentRoot(),"written-numbers-grammar.txt"));
		if (nluConfig.nlBusConfig!=null && !grammar.exists()) grammar=new File(Sanitizer.file(nluConfig.nlBusConfig.getContentRoot(),"common/nlu/written-numbers-grammar.txt"));
